...
const getAllDoctorService = async (params:any, options:pagination__interface)=>{
...
const updateDoctorservice = async(id:string,data:any)=>{
   ...
   const result = await prisma.$transaction(async (doctorClient)=>{
        ...
        for(let specialitiesId of specialties){
          const createSpecialities = await prisma.doctorSpecialties.create({
            data:{
              doctorId:doctorInfo.id,
              specialtiesId:specialitiesId
            }
          })
        }
